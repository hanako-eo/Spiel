var Spiel = (function (exports) {
	...
	***************************************************************************** */
	...
	// Function
	var LoaderEmitter = new events_1.EventEmitter();
	...
	(function (Entity) {
	    var Text = /** @class */ (function () {
	        ...
	        Text.prototype.afterRedraw = function () { };
	        Text.prototype.redraw = function () { };
	        Text.prototype.beforeRedraw = function () { };
	        Text.prototype.on = function (name, listener) {
	            if (name === "click")
	                click(this, listener);
	        };
	        Text.prototype.changeScene = function (name) { };
	        return Text;
	    }());
	    Entity.Text = Text;
	    ...
	    function Game(o, w, h) {
	        ...
	        var _loop_1 = function (entityName) {
	            o.load[entityName].then(function (l) {
	                ...
	                o.scene.forEach(function (scene) {
	                    if (entityName in scene.entity) {
	                        ...
	                        scene.entity[entityName].changeScene = function (name) {
	                            var i = o.scene.findIndex(function (scene) { return scene.name === name; });
	                            if (i !== -1)
	                                _this.use = o.scene[i].name;
	                        };
	                        if (l instanceof HTMLImageElement) {
	                            ...
	                            scene.entity[entityName].colide = function (entity) {
	    ...
	    Game.prototype.start = function (o) {
	        ...
	        window.onkeydown = function (ev) {
	            if (ev.key.includes("Arrow"))
	                ev.preventDefault();
	            _this.key.push([ev.ctrlKey ? "Ctrl" : null, ev.altKey ? "Alt" : null, ev.shiftKey ? "Shift" : null, ev.key === "Control" || ev.key === "Alt" || ev.key === "Shift" ? null : ev.key === "Delete" ? "Del" : ev.key === " " ? "Space" : ev.key]
	                .filter(function (keys) { return keys !== null; })
	                .join("+"));
	        };
	        window.onkeyup = function (ev) { return _this.key = _this.key.filter(function (v) { return v !== [ev.ctrlKey ? "Ctrl" : null, ev.altKey ? "Alt" : null, ev.shiftKey ? "Shift" : null, ev.key === "Control" || ev.key === "Alt" || ev.key === "Shift" ? null : ev.key === "Delete" ? "Del" : ev.key === " " ? "Space" : ev.key]
	            .filter(function (keys) { return keys !== null; })
	            .join("+"); }); };
	        var sceneId = o.scene.findIndex(function (scene) { return _this.use === scene.name; });
	        if (sceneId !== -1)
	            this.scene(o, sceneId);
	    };
	    Game.prototype.scene = function (o, sceneId) {
	        var _this = this;
	        this.entityList = o.scene[sceneId].entity;
	        setTimeout(function () {
	            for (var entityName in _this.entityList) {
	                if ("init" in _this.entityList[entityName])
	                    _this.entityList[entityName].init();
	                _this.draw(entityName);
	            }
	            var update = function () {
	                setTimeout(function () {
	                    _this.context.clearRect(0, 0, _this.w, _this.h);
	                ...
	                if (_this.use === o.scene[sceneId].name)
	                    requestAnimationFrame(update);
	                else {
	                    var sceneId_1 = o.scene.findIndex(function (scene) { return _this.use === scene.name; });
	                    if (sceneId_1 !== -1)
	                        _this.scene(o, sceneId_1);
	                }
	            };
	            requestAnimationFrame(update);
	        });
	    };
	    Game.prototype.draw = function (entityName) {
	        var _this = this;
	        if (entityName in this.load) {
	            var o = this.load[entityName];
	            if (o instanceof HTMLImageElement) {
	                if ("sprit" in this.entityList[entityName])
	                    this.context.drawImage(o, (this.entityList[entityName].entityWidth * this.entityList[entityName].sprit.x), (this.entityList[entityName].entityHeight * this.entityList[entityName].sprit.y), this.entityList[entityName].entityWidth, this.entityList[entityName].entityHeight, this.entityList[entityName].x, this.entityList[entityName].y, this.entityList[entityName].entityWidth * this.entityList[entityName].scale, this.entityList[entityName].entityHeight * this.entityList[entityName].scale);
	                else
	                    this.context.drawImage(o, this.entityList[entityName].x, this.entityList[entityName].y, this.entityList[entityName].entityWidth * this.entityList[entityName].scale, this.entityList[entityName].entityHeight * this.entityList[entityName].scale);
	            }
	            else if (!(o instanceof HTMLAudioElement)) {
	                var text_1 = o.text;
	                if ("replaced" in this.entityList[entityName])
	                    tslib_1.__spreadArrays([["", ""]], this.entityList[entityName].replaced).forEach(function (arr) {
	                        text_1 = text_1.replace.apply(text_1, arr).replace(/\*[a-z0-9]+/i, function (result) { return result.slice(1) in _this.entityList[entityName] ? _this.entityList[entityName][result.slice(1)] : result; });
	                    });
	                this.context.font = o.font;
	                this.context.fillText(text_1, this.entityList[entityName].x, this.entityList[entityName].y);
	            }
	        }
	    };
	    Game.prototype.update = function () {
	        for (var entityName in this.entityList) {
	            if (this.entityList[entityName] instanceof Entity.Image)
	                this.entityList[entityName].key = this.key;
	            if ("beforeRedraw" in this.entityList[entityName])
	                this.entityList[entityName].beforeRedraw();
	            if ("redraw" in this.entityList[entityName])
	                this.entityList[entityName].redraw();
	            this.draw(entityName);
	            if ("afterRedraw" in this.entityList[entityName])
	                this.entityList[entityName].afterRedraw();
	        }
	    };
	    return Game;
	}());
