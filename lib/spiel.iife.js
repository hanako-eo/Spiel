	...
	(function (Loader) {
	    ...
	    function Text(text, style) {
	        return new Promise(function (wait, fail) {
	            ...
	            var alpha = "alpha" in (style || {}) ? style.alpha : 0;
	            var padding = "padding" in (style || {}) ? style.alpha : 16;
	            setTimeout(function () {
	                LoaderEmitter.emit("loaded");
	                wait({
	                    fontSize: fontSize,
	...
	var Game = /** @class */ (function () {
	    function Game(o, w, h) {
	        ...
	        LoaderEmitter.on("loaded", function () {
	            if (++i === iL)
	                ...
	                o.loadScene(_this.context, (i / iL) * 100);
	            }
	        });
	    }
	    ...
	    Game.prototype.setEntity = function (o, scene, entityName, l) {
	        ...
	        if (!(scene.entity[entityName] instanceof Entity.Camera) && entityName !== "@camera") {
	            ...
	            scene.entity[entityName].getTarget = function () {
	                return _this.target[_this.sceneId];
	            };
	            scene.entity[entityName].setTarget = function (entity) {
	                if (scene.entity[entity] instanceof Entity.Camera)
	    ...
	    Game.prototype.draw = function (entityName, sceneId) {
	        ...
	        if (this.scenes[sceneId].backgroundColor)
	            this.canvas.style.backgroundColor = this.scenes[sceneId].backgroundColor;
	        if (entityName in this.load) {
	            ...
	            if (o_1 instanceof HTMLImageElement) {
	                ...
	                this.context.save();
	                this.context.font = o_1.fontSize + "px " + o_1.fontFamily;
	                this.context.globalAlpha = o_1.alpha;
	                this.context.fillStyle = o_1.color;
	                text_1.split("\n").forEach(function (text, i) {
	                    _this.context.fillText(text.trim(), _this.saveObject[sceneId][entityName].x + (_this.camera[sceneId] === undefined ? 0 : _this.camera[sceneId].x), _this.saveObject[sceneId][entityName].y + (_this.camera[sceneId] === undefined ? 0 : _this.camera[sceneId].y) + ((o_1.fontSize + o_1.padding) * i));
	                });
